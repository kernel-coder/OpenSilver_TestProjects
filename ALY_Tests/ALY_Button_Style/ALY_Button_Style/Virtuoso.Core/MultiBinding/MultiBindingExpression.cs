...
namespace Virtuoso.Core.MultiBinding
{
    ...
    public class MultiBindingExpression : INotifyPropertyChanged, IValueConverter, INotifyDataErrorInfo
    {
        ...
        public MultiBindingExpression(DependencyObject target, MultiBinding multiBinding)
        {
            ...
            {
                throw new ArgumentNullException("multiBinding");
            }
        ...
        private static DependencyProperty GetDataProperty(int dataPropertyIndex)
        {
            ...
            while (i <= dataPropertyIndex)
            {
                dataProperty = DependencyProperty.RegisterAttached("P" + i, typeof(Object),
                    typeof(MultiBindingExpression), new PropertyMetadata(null, OnDataPropertyChanged));
        ...
        private void SetDataProperty(int dataPropertyIndex, object localValue)
        {
            ...
            {
                BindingOperations.SetBinding(Target, destProperty, (Binding)localValue);
            }
            else
            {
            ...
            try
            {
                if (converter is IMultiValueConverter)
                {
                    ...
                    object[] convertedValues =
                        ((IMultiValueConverter)converter).ConvertBack(value, targetTypes, converterParameter, culture);
                ...
                if (converter is IValueConverter)
                {
                    object convertedValue =
                        ((IValueConverter)converter).ConvertBack(value, targetType, converterParameter, culture);
