...
namespace Virtuoso.Core.Services.MAR
{
     ...
     * 
     * */
    public class EncounterMARDataSet : IMARDataSet
    {
        ...
        internal EncounterMARDataSet(IPatientService model, Patient patient, Encounter encounter)
        {
            Model = model;
            Patient = patient;
            Encounter = encounter;
            ...
            */
            ...
            if (Encounter != null)
            {
                Encounter
                    ...
                    .Where(amm => amm.DocumentedState == MARDocumentState.UnTouched) // TODO: should we also add a check to only delete MAR data when the data has not been saved to the database?
        ...
        private bool MARProxyAllValid
        {
            get
            {
                return ((MARProxyEncounterShift == null) || (MARProxyEncounterStartDate == null)) ? false : true;
            }
        }
        ...
        // TODO refactor this.  Roll PRN filter into GetMARMedicationList - no need for this yield abstraction, now that 
        // GetMARMedicationList has changed to LINQ with predicate for filter
        private IEnumerable<PatientMedication> Get_Medication_For_MAR(bool isPRN = false, bool returnDiscontinuedMedications = false)
        {
            IMARMedicationView marMedView = MARUtils.CreateCurrentMedicationsFilter(Patient, null, Encounter, false, null);
            ...
            if (MARProxyAllValid) // don't attempt to populate the MAR until we have date/time ranges to do so
            {
                         ...
                         meds_for_encounter) // NOTE: Some of these meds may be discontinued!  Skip those when building the MAR list unless that med has a MAR for this encounter!
                {
                        ...
                        (newMARMed.MARPatientMedication.AdmissionMedicationMAR != null))
                    {
                        ...
                        if (pm.MedicationStatus != 2 || pm.AdmissionMedicationMAR.Any(amm => amm.AddedFromEncounterKey == Encounter.EncounterKey))
                        {
                            MARUtils.Refresh_PRN(newMARMed, false, null, Encounter);
                        }
                    }
                }
            }
        ...
        private List<MARMed> Refresh_STD_List()
        {
            ...
            if (MARProxyAllValid) // don't attempt to populate the MAR until we have date/time ranges to do so
            {
                ...
                // BFM - need new algorythm here if Admission Mode
                // need to use a list of current meds (not meds_for_encounter)
                // and a second version of Refresh_STD(newMARMed) that uses
                // MARGetAdmissionMedicationMARsForMed to get all MARs for the med - then only include those of the given ServiceDate 
        ...
        private void Refresh_STD(MARMed MARMed)
        {
            ...
            admissionMedicationMAR_For_PatientMedication = MARMed.MARPatientMedication
                .AdmissionMedicationMAR
                .Where(amm => amm.AddedFromEncounterKey == Encounter.EncounterKey)
        ...
        private void AddAdmissionMedicationMARorShift(PatientMedication pm, List<AdmissionMedicationMAR> ammList, MARMed newMARMed)
        {
            // TODO JE what to do for IsAdmissionMode
