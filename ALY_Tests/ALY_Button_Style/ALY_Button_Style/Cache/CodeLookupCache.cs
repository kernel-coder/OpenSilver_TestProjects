    ...
    public class CodeLookupCache : ReferenceCacheBase<CodeLookupHeader>
    {
            ...
            int plusMeKey = 0)
        {
            ...
            if (sequence == null)
            {
                if (includeInactive)
                {
                    var ret = (orderByCodeDescription)
                        ? codeLookupType.CodeLookup.OrderBy(c => c.CodeDescription).ThenBy(c => c.Sequence).ToList()
                        : codeLookupType.CodeLookup.OrderBy(c => c.Code).ThenBy(c => c.Sequence).ToList();
                    if (includeEmpty)
                    {
                        ret.Insert(0, new CodeLookup { Code = " ", CodeDescription = " ", CodeLookupKey = 0 });
                    }
                ...
                {
                    var ret = (orderByCodeDescription)
                        ? codeLookupType.CodeLookup
                            .Where(c => ((c.Inactive == false) || (c.CodeLookupKey == plusMeKey)))
                            .OrderBy(c => c.CodeDescription).ThenBy(c => c.Sequence).ToList()
                        : codeLookupType.CodeLookup
                            .Where(c => ((c.Inactive == false) || (c.CodeLookupKey == plusMeKey)))
                            .OrderBy(c => c.Code)
            ...
            {
                var ret = (orderByCodeDescription)
                    ? codeLookupType.CodeLookup.Where(c => c.Sequence <= sequence).OrderBy(c => c.CodeDescription)
                        .ThenBy(c => c.Sequence).ToList()
                    : codeLookupType.CodeLookup.Where(c => c.Sequence <= sequence).OrderBy(c => c.Code)
                        .ThenBy(c => c.Sequence).ToList();
                if (includeEmpty)
                {
                    ret.Insert(0, new CodeLookup { Code = " ", CodeDescription = " ", CodeLookupKey = 0 });
            ...
            {
                var ret = (orderByCodeDescription)
                    ? codeLookupType.CodeLookup.Where(c => ((c.Inactive == false) || (c.CodeLookupKey == plusMeKey)))
                        .Where(c => c.Sequence <= sequence).OrderBy(c => c.CodeDescription).ThenBy(c => c.Sequence)
                        .ToList()
                    : codeLookupType.CodeLookup.Where(c => ((c.Inactive == false) || (c.CodeLookupKey == plusMeKey)))
                        .Where(c => c.Sequence <= sequence).OrderBy(c => c.Code).ThenBy(c => c.Sequence).ToList();
                if (includeEmpty)
                {
                    ret.Insert(0, new CodeLookup { Code = " ", CodeDescription = " ", CodeLookupKey = 0 });
                }
            ...
            int plusMeKey = 0)
        {
            ...
            if (sequence == null)
            {
                ...
                {
                    var ret = (orderByCodeDescription)
                        ? codeLookupType.CodeLookup.Where(c => c.ApplicationDataContains(applicationData))
                            .OrderBy(c => c.CodeDescription).ThenBy(c => c.Sequence).ToList()
                        : codeLookupType.CodeLookup.Where(c => c.ApplicationDataContains(applicationData))
                            .OrderBy(c => c.Code).ThenBy(c => c.Sequence).ToList();
                    if (includeEmpty)
                    {
                        ret.Insert(0, new CodeLookup { Code = " ", CodeDescription = " ", CodeLookupKey = 0 });
                    }
                ...
                else
                {
                    var ret = (orderByCodeDescription)
                        ? codeLookupType.CodeLookup
                            .Where(c => (((c.Inactive == false) && (c.ApplicationDataContains(applicationData))) ||
                                         (c.CodeLookupKey == plusMeKey))).OrderBy(c => c.CodeDescription)
                            .ThenBy(c => c.Sequence).ToList()
                        : codeLookupType.CodeLookup
                            .Where(c => (((c.Inactive == false) && (c.ApplicationDataContains(applicationData))) ||
                                         (c.CodeLookupKey == plusMeKey))).OrderBy(c => c.Code).ThenBy(c => c.Sequence)
                            .ToList();
                    if (includeEmpty)
                    {
                        ret.Insert(0, new CodeLookup { Code = " ", CodeDescription = " ", CodeLookupKey = 0 });
            ...
            if (includeInactive)
            {
                var ret = (orderByCodeDescription)
                    ? codeLookupType.CodeLookup
                        .Where(c => ((c.Sequence <= sequence) && (c.ApplicationDataContains(applicationData))))
                        .OrderBy(c => c.CodeDescription).ThenBy(c => c.Sequence).ToList()
                    : codeLookupType.CodeLookup
                        .Where(c => ((c.Sequence <= sequence) && (c.ApplicationDataContains(applicationData))))
                        .OrderBy(c => c.Code).ThenBy(c => c.Sequence).ToList();
            ...
            else
            {
                var ret = (orderByCodeDescription)
                    ? codeLookupType.CodeLookup
                        .Where(c => (((c.Inactive == false) && (c.ApplicationDataContains(applicationData))) ||
                                     (c.CodeLookupKey == plusMeKey))).Where(c => c.Sequence <= sequence)
                        .OrderBy(c => c.CodeDescription).ThenBy(c => c.Sequence).ToList()
                    : codeLookupType.CodeLookup
                        .Where(c => (((c.Inactive == false) && (c.ApplicationDataContains(applicationData))) ||
                                     (c.CodeLookupKey == plusMeKey))).Where(c => c.Sequence <= sequence)
            ...
            int plusMeKey = 0)
        {
            ...
            {
                var ret = (orderByCodeDescription)
                    ? codeLookupType.CodeLookup.Where(c => c.Sequence <= sequence)
                        .OrderBy(c => c.CodeDescription)
                        .ThenBy(c => c.Sequence).ToList()
                    : codeLookupType.CodeLookup.Where(c => c.Sequence <= sequence)
                        .OrderBy(c => c.Code)
