...
{
    [ExportMetadata("CacheName", ReferenceTableName.ICDCM9)]
    [Export(typeof(ICache))]
    public class ICDCM9Cache : FlatFileCacheBase<CachedICDCode>
    {
        public static ICDCM9Cache Current { get; private set; }

        [ImportingConstructor]
        public ICDCM9Cache(ILogger logManager)
            : base(logManager, true)
        {
            if (Current == this)
            {
                throw new InvalidOperationException("ICDCM9Cache already initialized.");
            }

            Current = this;
            CacheName = ReferenceTableName.ICDCM9;
            DatabaseWrapper = DatabaseService.Current.DatabaseFor(VirtuosoDatabase.ICDCM9);
        }
