...
namespace Virtuoso.Server.Data
{
    ...
    public partial class Admission : IServiceLineGroupingService, IEncounterAdmission, INotifyDataErrorInfo
    {
        ...
        public AdmissionSiteOfService GetAdmissionSiteOfService(DateTime? pEffectiveDateTime, bool isAdmissionMode = false)
        {
            ...
            AdmissionSiteOfService asos = null;
            if (isAdmissionMode)
            {
                // When View MAR is accessed from Admission Maintenance, return the first valid for MAR 
                // site of service from the past.  E.G. pEffectiveDateTime <= DateTime.Now
                // BPI #58849 View MAR when the patient was at a facility in the past
                asos = AdmissionSiteOfService
                .Where(sos => sos.HistoryKey == null)
                .Where(sos => sos.SiteOfServiceFromDateTimeHasValue)
                .Where(sos => sos.SiteOfServiceFromDateTimeSort <= effectiveDateTime)
                .Where(sos => sos.SiteOfServiceIsApplicableToMAR == true)
                .OrderBy(sos => sos.SiteOfServiceFromDateTimeSort)
                .FirstOrDefault();
            }
            else
            {
                // When View MAR is accessed in a visit note, return the first valid site of service that is effective for 
                // pEffectiveDateTime (Encounter StartDate)
                asos = AdmissionSiteOfService.Where(sos =>
        ...
        public bool AdmissionSiteOfServiceIsApplicableToMAR(DateTimeOffset? pEffectiveDateTime, bool isAdmissionMode = false)
        {
            var effectiveDateTime = pEffectiveDateTime == null
                ? DateTime.SpecifyKind(DateTime.Now, DateTimeKind.Unspecified)
                : ((DateTimeOffset)pEffectiveDateTime).DateTime;
            var asos = GetAdmissionSiteOfService(effectiveDateTime, isAdmissionMode);
            if (asos == null)
            {
                return false;
            }
