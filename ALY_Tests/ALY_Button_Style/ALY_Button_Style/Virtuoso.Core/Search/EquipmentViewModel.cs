...
namespace Virtuoso.Core.Search
{
    ...
    [ExportMetadata("FieldName", "EquipmentSearch")]
    [Export(typeof(ISearch))]
    public class EquipmentViewModel : GenericBase, ISearch
    {
        public IEquipmentService Model { get; set; }
        ...
        public EquipmentViewModel(IEquipmentService _model)
        {
            SelectCommand = new RelayCommand<Equipment>(p =>
            {
                SelectAction?.Invoke();
            });
            ...
            Equipments.SortDescriptions.Add(new SortDescription("Description",
                ListSortDirection.Ascending)); //initial sort ordering...must match a bound property in the XAML
        }

        private CollectionViewSource _Equipments = new CollectionViewSource();

        public CollectionViewSource Equipments
        {
        ...
        public RelayCommand<Equipment> SelectCommand { get; protected set; }
        ...
        void OnLoaded(object sender, EntityEventArgs<Equipment> e)
        {
            if (e.Error == null)
            {
                Equipments.Source = e.Results.ToList();

                this.RaisePropertyChangedLambda(p => p.Equipments);
                this.RaisePropertyChangedLambda(p => p.TotalRecords);
            }
            else
            {
                ErrorDetailLogger.LogDetails(e, true, "Virtuoso.Core.Search.EquipmentViewModel.OnLoaded");
            }
        ...
        public int TotalRecords => Model.Equipments.Count;

        public string SelectedField => "EquipmentKey";
        ...
        public string SelectedValue
        {
            get
            {
                ...
                return ((Equipment)SelectedItem).EquipmentKey.ToString();
            }
        }
