...
{
    [ExportMetadata("CacheName", ReferenceTableName.ICDPCS10)]
    [Export(typeof(ICache))]
    public class ICDPCS10Cache : FlatFileCacheBase<CachedICDCode>
    {
        public static ICDPCS10Cache Current { get; private set; }

        [ImportingConstructor]
        public ICDPCS10Cache(ILogger logManager)
            : base(logManager, true)
        {
            if (Current == this)
            {
                throw new InvalidOperationException("ICDPCS10Cache already initialized.");
            }

            Current = this;
            CacheName = ReferenceTableName.ICDPCS10;
            DatabaseWrapper = DatabaseService.Current.DatabaseFor(VirtuosoDatabase.ICDPCS10);
        }
