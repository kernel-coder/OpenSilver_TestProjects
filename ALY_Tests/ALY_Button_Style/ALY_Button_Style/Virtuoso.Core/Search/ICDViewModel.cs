...
namespace Virtuoso.Core.Search
{
    ...
    [ExportMetadata("FieldName", "ICDSearch")]
    [Export(typeof(ISearch))]
    public class ICDViewModel : GenericBase, ISearch
    {
        public IICDCodeService Model { get; set; }
        ...
        public ICDViewModel(IICDCodeService _model)
        {
            SelectCommand = new RelayCommand<ICDCode>(p =>
            {
                SelectAction?.Invoke();
            });
            ...
            ICDCodes.SortDescriptions.Add(new SortDescription("Code",
                ListSortDirection.Ascending)); //initial sort ordering...must match a bound property in the XAML
        }

        private CollectionViewSource _ICDCodes = new CollectionViewSource();

        public CollectionViewSource ICDCodes
        {
            get { return _ICDCodes; }
            set
            {
                _ICDCodes = value;
                RaisePropertyChanged("ICDCodes");
            }
        }

        public RelayCommand<ICDCode> SelectCommand { get; protected set; }
        ...
        void OnLoaded(object sender, EntityEventArgs<ICDCode> e)
        {
            if (e.Error == null)
            {
                ICDCodes.Source = e.Results.ToList();

                this.RaisePropertyChangedLambda(p => p.ICDCodes);
                this.RaisePropertyChangedLambda(p => p.TotalRecords);
            }
            else
            {
                ErrorDetailLogger.LogDetails(e, true, "Virtuoso.Core.Search.ICDViewModel.OnLoaded");
            }
        ...
        public int TotalRecords => Model.ICDCodes.Count;

        public string SelectedField => "ICDCodeKey";
        ...
        public string SelectedValue
        {
            get
            {
                ...
                return ((ICDCode)SelectedItem).ICDCodeKey.ToString();
            }
        }
