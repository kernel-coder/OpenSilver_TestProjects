...
{
    public partial class SignatureDataWithDateUserControl : UserControl, INotifyPropertyChanged
    {
        public SignatureDataWithDateUserControl()
        {
            // Required to initialize variables
            InitializeComponent();
            IsPrint = false;
            this.Loaded += SignatureDataWithDateUserControl_Loaded;
        }
        void SignatureDataWithDateUserControl_Loaded(object sender, RoutedEventArgs e)
        {
        ...
        public static readonly DependencyProperty EncounterDataProperty =
            ...
            typeof(SignatureDataWithDateUserControl), new PropertyMetadata(new PropertyChangedCallback(EncounterDataChanged)));

        private static void EncounterDataChanged(DependencyObject sender, DependencyPropertyChangedEventArgs args)
        {
            try
            {
                var ctrl = sender as SignatureDataWithDateUserControl;
                ctrl.SetSignature();
            }
            catch (Exception oe)
            {
        ...
        public static readonly DependencyProperty IsPrintProperty =
            DependencyProperty.Register("IsPrint", typeof(Boolean), typeof(SignatureDataWithDateUserControl), new PropertyMetadata(IsPrintPropertyChanged));

        private static void IsPrintPropertyChanged(object sender, DependencyPropertyChangedEventArgs args)
        {
            SignatureDataWithDateUserControl uc = sender as SignatureDataWithDateUserControl;
            if (uc != null)
            {
        ...
        public bool IsPrint
        {
            get { return ((bool)(base.GetValue(Virtuoso.Core.Controls.SignatureDataWithDateUserControl.IsPrintProperty))); }
            set { base.SetValue(Virtuoso.Core.Controls.SignatureDataWithDateUserControl.IsPrintProperty, value); }
        }
        ...
        private void ConvertToImage()
        {
            ...
            if (EncounterData != null)
            {
                ...
                if (EncounterData.SignatureData != null) EncounterData.DateTimeData = DateTime.SpecifyKind(DateTime.Today, DateTimeKind.Unspecified).Date;
                this.RaisePropertyChanged("Signature");
            }
        }
        ...
        private void signatureDataWithDateUserControl_ctrl_BindingValidationError(object sender, ValidationErrorEventArgs e)
        {
